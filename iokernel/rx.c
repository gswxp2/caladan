...
static bool rx_send_pkt_to_runtime(struct proc *p, struct tx_net_hdr *hdr)
{
	...
	if (unlikely(htons(*(uint16_t *)(hdr->payload + 34)) == 601))
	{
		times[time_count++] = rte_rdtsc();
	}
...
static void rx_one_pkt(struct tx_net_hdr *buf)
{
	...
	ptr_mac_hdr = (struct rte_ether_hdr *)buf->payload;
	ptr_dst_addr = &ptr_mac_hdr->d_addr;
	log_debug_ratelimited("rx: rx packet with MAC %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8,
...
bool rx_burst(void)
{
	struct tx_net_hdr *bufs[IOKERNEL_RX_BURST_SIZE];
	uint16_t nb_rx, i;
	...
	// if (nb_rx > 0)
	// 	printf("rx: received %d packets on port %d", nb_rx, dp.port);

	for (i = 0; i < nb_rx; i++) {
		if (i + RX_PREFETCH_STRIDE < nb_rx) {
			prefetch(bufs[i + RX_PREFETCH_STRIDE]->payload);
		}
		rx_one_pkt(bufs[i]);
	}
